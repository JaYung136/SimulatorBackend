...
import org.sim.workflowsim.MigrateHostSelector;
...
public class VmAllocationPolicySimple extends VmAllocationPolicy {
	...
	public List<Map<String, Object>> optimizeAllocation(List<? extends Vm> vmList) {
		...
		MigrateHostSelector selector = new MigrateHostSelector();
		List<Integer> hosts = new ArrayList<>();
		for(Host h: overUtilizedHosts) {
			hosts.add(h.getId());
		}
		int num = 0;
		//Log.printLine("OverUtiliedHost:");
		for(Host h: overUtilizedHosts) {
			Map<String, Object> mg = new HashMap<>();
			ResCloudlet rcl = h.choseCloudletToMigrate();
			if(rcl.getCloudlet().lastVmId != -1)
				continue;
			Host target = selector.findHostForJob(hosts);
			if(rcl == null) {
				continue;
			}
			if(target == null) {
			...
			mg.put("container", rcl);
			mg.put("targetHost", target);
			mg.put("oldHost", h);
			migrationMap.add(mg);
			num ++;
			if(num >= Constants.migrateNum)
				break;
		}
	...
	/*
	 * (non-Javadoc)
	 * @see VmAllocationPolicy#allocateHostForVm(Vm,
	 * Host)
	 */
	@Override
