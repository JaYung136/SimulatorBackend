...
import org.sim.cloudbus.cloudsim.lists.PeList;
import org.sim.cloudbus.cloudsim.provisioners.BwProvisioner;
import org.sim.cloudbus.cloudsim.provisioners.RamProvisioner;
 ...
 */
public class Host {
	public Integer getUsedPe() {
		return ((CloudletSchedulerTimeShared)getCloudletScheduler()).getExecSize();
	}
	private String name;
	...
	public void setCloudletScheduler(CloudletScheduler cloudletScheduler) {
		this.scheduler = cloudletScheduler;
		List<Double> mips = new ArrayList<>();
		for(Pe p: getPeList()) {
			mips.add(p.getPeProvisioner().getMips());
		}
		getCloudletScheduler().updateVmProcessing(CloudSim.clock(), mips, getRamProvisioner().getRam());
	}

	public double getUtilizationOfCpu() {
		return getCloudletScheduler().getCpuUtilization();
	}

	public double getPreviousCpu() {
		return previousCpu;
	}

	public double getUtilizationOfRam() {
		return getCloudletScheduler().getRamUtilization();
	}

	public double getPreviousRam() {
		return previousRam;
	}
	...
	public double updateVmsProcessing(double currentTime) {
		if(count > 0) {
			this.previousCpu = getCloudletScheduler().getCpuUtilization();
			this.previousRam = getCloudletScheduler().getRamUtilization();
		}
		count ++;
		...
		double time = getCloudletScheduler().updateVmProcessing(CloudSim.clock(), mips, getRamProvisioner().getRam());
	...
	public ResCloudlet choseCloudletToMigrate() {
		return getCloudletScheduler().choseCloudletToMigrate();
	}
	/**
	...
	public boolean vmCreate(Vm vm) {
		vm.setMips(getVmScheduler().getPeCapacity());
		if (getStorage() < vm.getSize()) {
			Log.printLine("[ContainerScheduler.containerCreate] Allocation of Container #" + vm.getId() + " to Host #" + getId()
					+ " failed by storage");
			return false;
		}
	...
	public double findContainerFinishTime(Task task, Double time, Boolean occupySlot) {
		CondorVM containerTmp = new CondorVM(task.getCloudletId(), 1, getVmScheduler().getPeCapacity(), task.getNumberOfPes(), (int) task.getRam(), 0, 0, "Xen", new CloudletSchedulerTimeShared());
		// 如果没有足够的资源创建容器，这时我们默认容器会持续运行
		if(!vmCreate(containerTmp)) {
			return Double.MAX_VALUE;
		}
		if(!occupySlot) {
			vmDeallocate(containerTmp);
		}
		return time + task.getCloudletLength() / getVmScheduler().getPeCapacity();
	}
