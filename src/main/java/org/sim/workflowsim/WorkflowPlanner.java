...
import org.sim.workflowsim.planning.BasePlanningAlgorithm;
import org.sim.workflowsim.planning.HEFTPlanningAlgorithm;
import org.sim.workflowsim.utils.Parameters;
import org.sim.workflowsim.utils.Parameters.PlanningAlgorithm;
...
public final class WorkflowPlanner extends SimEntity {
    ...
    public void rewriteAppXml() throws Exception{
        try {
            util.rewriteAppInfoXml(Constants.appFile);
        }catch (Exception e) {
            throw e;
        }
    }

    public void setAppPath(String p) {
        this.appPath = p;
    }
    ...
    public void processEvent(SimEvent ev) {
        switch (ev.getTag()) {
            case WorkflowSimTags.START_SIMULATION:
                ...
                }else if(Parameters.getPlanningAlgorithm() == PlanningAlgorithm.HEFT){
                    util = new XmlUtil(getClusteringEngine().getWorkflowEngine().getSchedulerId(0));
                    util.parseHostXml(Constants.appFile);
                    setTaskList(util.getTaskList());
                    this.taskList = util.getTaskList();
